...
const checks = {
  has35References: references.length === 35,
  has35UniqueUrls: new Set(references.map(reference => canonicalPublicUrl(reference.sourceUrl))).size === 35,
...
if (failed.length) throw new Error(`Verificação v3 falhou: ${failed.join(", ")}`);
console.log(JSON.stringify({ ok: true, checks, references: references.length }, null, 2));

  ...
  json_schema: {
    ...
    schema: {
      ...
      properties: {
        ...
        hypotheses: { type: "array", maxItems: 5, items: { type: "object", properties: { name: { type: "string" }, targetPatternId: { anyOf: [{ type: "integer" }, { type: "string" }, { type: "null" }] }, evidenceRole: { type: "string", enum: ["support", "counterexample", "case_limit", "context"] }, comparisonLevel: { type: "integer", minimum: 1, maximum: 4 }, requiredEvidenceObserved: { type: "boolean" }, patternType: { type: "string", enum: [...PATTERN_TYPES] }, observation: { type: "string" }, mechanism: { type: "string" }, evidence: { type: "string" }, alternativeExplanations: { type: "array", items: { type: "string" } }, conditions: { type: "array", items: { type: "string" } }, limitations: { type: "array", items: { type: "string" } }, confidence: { type: "string", enum: ["low", "medium", "high"] }, stage: { type: "string", enum: ["observation", "hypothesis", "contradicted", "inconclusive"] } }, required: ["name", "targetPatternId", "evidenceRole", "comparisonLevel", "requiredEvidenceObserved", "patternType", "observation", "mechanism", "evidence", "alternativeExplanations", "conditions", "limitations", "confidence", "stage"], additionalProperties: false } },

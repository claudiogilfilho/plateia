...
export const CONSUMERS = [
  ...
  "O Analítico",
  "O Aspiracional",
  "O Influenciado pela Comunidade",
  "O Cético",
] as const;
...
export type ContentAnalysis = {
  consumers: Array<{
    ...
    overallScore: number;
    reaction: string;
    criteria: Record<(typeof CRITERIA)[number], number>;
      ...
      properties: {
        consumers: {
          ...
          items: {
            type: "object",
            properties: {
              name: { type: "string", enum: [...CONSUMERS] },
              overallScore: { type: "integer", minimum: 0, maximum: 100 },
              reaction: { type: "string" },
              criteria: {
                type: "object",
                properties: {
                  gancho: { type: "integer", minimum: 0, maximum: 100 },
                  clareza: { type: "integer", minimum: 0, maximum: 100 },
                  relevância: { type: "integer", minimum: 0, maximum: 100 },
                  desejo: { type: "integer", minimum: 0, maximum: 100 },
                  confiança: { type: "integer", minimum: 0, maximum: 100 },
                  retenção: { type: "integer", minimum: 0, maximum: 100 },
                  ação: { type: "integer", minimum: 0, maximum: 100 },
                  objeções: { type: "integer", minimum: 0, maximum: 100 },
                },
                required: [...CRITERIA],
            ...
            required: ["name", "overallScore", "reaction", "criteria", "mainObjection"],
            additionalProperties: false,
          },
        },
        synthesis: {
          ...
          properties: {
            overallScore: { type: "integer", minimum: 0, maximum: 100 },
            weightedAverage: { type: "integer", minimum: 0, maximum: 100 },
          ...
          required: ["overallScore", "weightedAverage", "divergence", "strengths", "risks", "recommendations"],
          additionalProperties: false,
        },
      },
...
export function validateAnalysisShape(analysis: ContentAnalysis): ContentAnalysis {
  if (analysis.consumers.length !== CONSUMERS.length) {
    throw new Error("A análise precisa conter os cinco consumidores sintéticos.");
  }

  const consumerNames = new Set(analysis.consumers.map(consumer => consumer.name));
  if (consumerNames.size !== CONSUMERS.length || CONSUMERS.some(name => !consumerNames.has(name))) {
    throw new Error("Os consumidores retornados não correspondem à configuração da Platéia.");
  }

  if (analysis.synthesis.recommendations.length !== 3) {
    throw new Error("A Platéia precisa retornar exatamente três recomendações prioritárias.");
  }
...
function recordOf(value: unknown, field: string): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`${field} inválido na resposta da IA.`);
  return value as Record<string, unknown>;
}

function textOf(value: unknown, field: string) {
  if (typeof value !== "string" || !value.trim()) throw new Error(`${field} inválido na resposta da IA.`);
  return value.trim();
}

function scoreOf(value: unknown, field: string) {
  const score = Number(value);
  if (!Number.isFinite(score)) throw new Error(`${field} precisa ser uma nota numérica.`);
  return Math.round(Math.max(0, Math.min(100, score)));
}

function listOf(value: unknown, field: string, min: number, max: number) {
  if (!Array.isArray(value) || value.length < min || value.length > max) throw new Error(`${field} possui quantidade inválida de itens.`);
  return value.map((item, index) => textOf(item, `${field} ${index + 1}`));
}
...
export function normalizeAnalysis(input: unknown): ContentAnalysis {
  ...
  const consumers = CONSUMERS.map(name => {
    ...
    const criteria = Object.fromEntries(CRITERIA.map(criterion => [criterion, scoreOf(rawCriteria[criterion], `${criterion} de ${name}`)])) as Record<(typeof CRITERIA)[number], number>;
    return {
      name,
      overallScore: scoreOf(consumer.overallScore, `Nota geral de ${name}`),
      reaction: textOf(consumer.reaction, `Reação de ${name}`),
      criteria,
      mainObjection: textOf(consumer.mainObjection, `Objeção de ${name}`),
    };
  });

  const rawSynthesis = recordOf(raw.synthesis, "Síntese");
  const recommendations = listOf(rawSynthesis.recommendations, "Recomendações", 3, 3) as [string, string, string];
  return validateAnalysisShape({
    consumers,
    synthesis: {
      overallScore: scoreOf(rawSynthesis.overallScore, "Nota geral"),
      weightedAverage: scoreOf(rawSynthesis.weightedAverage, "Média ponderada"),
      divergence: scoreOf(rawSynthesis.divergence, "Divergência"),
      strengths: listOf(rawSynthesis.strengths, "Pontos fortes", 2, 4),
      risks: listOf(rawSynthesis.risks, "Riscos", 2, 4),
      recommendations,
    },
  });
}

export async function evaluateContent(input: {
...
Produto relacionado: ${input.product}
